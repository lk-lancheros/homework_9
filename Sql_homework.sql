...
-- 1b. Display from table 'actor' first/last name, single column, upper case letters. `Actor Name`--

SELECT CONCAT (UPPER(first_name),' ', UPPER(last_name)) as 'Actor Name'
FROM actor
ORDER BY first_name ASC;

-- 2a. Find from table 'actor' the actor ID number, first name ("JOE"), last name --

SELECT actor_ID, first_name as 'Actor: First name', last_name as 'Actor: Last name'
FROM actor
WHERE first_name='JOE';

-- 2b. Find from table 'actor' all actors, last name contain `GEN`--
...
-- 2c. Find from table 'actor' all actors, last names contain `LI` --
-- Order the rows by last name and first name, in that order --

SELECT first_name as 'Actor: First name', last_name as 'Actor: Last name contains "LI"'
FROM actor
WHERE last_name LIKE '%LI%'
ORDER BY last_name ASC, first_name ASC;
 
-- 2d. Using table 'country' and `IN`, display `country_id`/`country` columns Afghanistan, Bangladesh, and China --
...
-- 3b. Delete from table 'actor' the `description` column --

ALTER TABLE actor
DROP description;

-- 4a. From table 'actor' list the last names of actors, as well as how many actors have that last name. --
...
-- 4b.From table 'actor' list last names/number of actors w/ last name shared by 2+ actors --
...
-- 6d. Using tables 'inventory' and 'film' count inventory_id from 'table' inventory --
-- how many of the film `Hunchback Impossible` exist in the inventory system? --
...
-- 7b.  Use tables 'actor', 'film_actor', 'film' and subqueries --
...
-- 7d. Identify all movies categorized as _family_ films using tables 'film', 'film_category', 'category' --
...
-- 7e. Display the most frequently rented movies count of rental_id from table 'rental' in descending order. --
-- Use tables 'rental', 'inventory', 'film' --
...
-- 7f. Write a query to display how much business, in dollars or sum(amount) from table 'payment', each store brought in. --
-- Use tables 'payment', 'staff', 'store', 'address', 'city', 'country' and sort by sum(payment.amount) --
...
-- Use tables 'store', 'city', 'country' and sort by store_id --
...
-- 7h. List the top five genres(category name) in gross revenue or sum(amount) from table 'payment' in descending order. --
-- Use tables 'category', 'film_category', 'inventory', 'rental', 'payment' --
